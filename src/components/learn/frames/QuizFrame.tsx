      ...
      <div className="w-full max-w-sm flex flex-col gap-4">
        ...
        <div className="space-y-2">
          {card.quiz.options.map((opt, oi) => {
            let cls = "bg-black/50 border-zinc-700 text-zinc-100 hover:border-zinc-500";
            if (answered) {
              if (oi === card.quiz.correctIndex)
                cls = "bg-emerald-500/20 border-emerald-500 text-emerald-200";
              else if (oi === answer) cls = "bg-rose-500/20 border-rose-500 text-rose-300";
              else cls = "bg-black/30 border-zinc-800 text-zinc-500";
            }
            ...
            return (
              <button
                ...
                disabled={answered}
                onClick={() => onAnswer(oi)}
                className={`w-full text-left text-xs font-semibold px-3 py-3 rounded-xl border transition-colors ${cls}`}
              >
                <span className="mr-2 text-zinc-400 font-mono">
